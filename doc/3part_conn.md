...
TrayBot 采用**端云分离**架构：Agent 与 Dashboard **不直连**，所有实时数据经 Backend 中转。
...
| **Backend** | 云服务器 | 工单队列权威源；维护 Dashboard 运行时状态；Agent ↔ Dashboard 消息转发中枢 |
| **Dashboard** | 浏览器 | 连接 Backend WebSocket；展示工单池、图文直播、地图导航、本体状态；**不**直接与 Agent 通信 |

### 1.2 设计原则

1. **Backend 是唯一中枢**：Dashboard 只订阅 Backend；Agent 只上报 Backend。
2. **工单权威在 Backend**：工单创建、排队、状态变更由 `WorkOrderStore` 管理；Agent 只执行被分派的工单。
3. **全局单工单执行**：同一时刻最多 1 条 `in_progress` 工单、1 个 Agent 连接。
4. **协议共享**：Agent 与 Backend 共用 `shared/traybot_protocol` 包，类型与 action 常量保持一致。
...
| 端 | 配置 |
|----|------|
| Agent | `python -m app.main run-cloud`（`--transport mqtt`，Broker 默认 `127.0.0.1:1883`） |
| Backend | 启动时自动连接 Broker（`TRAYBOT_MQTT_ENABLED=true`，见 `backend/app/mqtt_bridge.py`） |
...
三端 WebSocket 消息统一采用 JSON Envelope：
...
Backend `ConnectionHub`（`backend/app/hub.py`）负责接收 Agent 消息并转换为 Dashboard 消息广播。
...
| action | 触发时机 |
|--------|----------|
| `workorder.assign` | 新建首单 / 上一单完成 / Agent 重连且仍有执行中工单 |
| `ping` | Backend 心跳（可选） |
...
```
Agent                        Backend (MqttBridge)
    │                           │
    │── connect MQTT Broker ───▶│  （Backend 已订阅 thing/product/traybot/+/osd）
    │── publish osd: hello ────▶│
    │                           │
    │◀── service: workorder.assign ─│  （若有 in_progress 工单）
    │                           │
    │  … 执行工作流，持续 publish osd … │
```
...
- 同时只允许 **1 个** Agent；新连接会 close 旧连接（code `4000`）。
- Agent 重连时，若 `_executing_order_id` 仍在执行 → 重新发送 `workorder.assign`。
- Agent 启动有文件锁 `/tmp/traybot-agent.lock`，禁止多实例并行。
...
```
1. Backend ──workorder.assign──▶ Agent
2. Agent 执行 LangGraph 工作流，逐步上报 agent.event / agent.state
3. 放架成功时 ──agent.workorder.progress──▶ Backend ──workorder.updated──▶ Dashboard
...
Thinking 内容**不在** `agent.event` 中携带全文，而是分帧推送：

```
1. agent.event          → event.created     （事件骨架，无 thinking）
2. agent.thinking.delta → event.thinking.delta  × N（0.04s/字）
...
Agent 通过 `agent.state` 上报，Backend 转发为 `state.patch`：
...
**源码**：`agent/app/reporter.py`、`agent/app/runner.py`

| 行为 | 实现 |
|------|------|
| 连接 | `CloudReporter.connect()` → 发 `agent.hello` |
| 事件 | `publish_event()` → `agent.event` + 逐字 `thinking.delta` + `thinking.done` |
...
**源码**：`backend/app/state.py`、`backend/app/work_orders.py`

| 组件 | 职责 |
|------|------|
| `WorkOrderStore` | 工单池 CRUD、排队、`complete()` 自动启动下一单 |
| `DashboardState` | `live_events`（最多 40 条）、`robot`、`map_state` |
| `ConnectionHub` | WebSocket 连接管理、消息路由、`_executing_order_id` 执行锁 |
...
| `shared/traybot_protocol/models.py` | LiveEvent、WorkOrder 模型 |
| `agent/app/reporter.py` | Agent WebSocket 客户端 |
| `agent/app/runner.py` | 工作流逐步执行 + 上报 |
| `agent/app/map_state.py` | 事件 → 地图状态映射 |
| `backend/app/hub.py` | ConnectionHub 转发中枢 |
| `backend/app/server.py` | FastAPI 入口 + REST + WS 路由 |
...
## 15. 不在 WebSocket 范围内的能力

以下能力**不走**三方 WebSocket 协议：
