...
端云分离的机器人监控与作业管理系统：端侧 Agent 执行 LangGraph 工作流，云端 Backend 管理工单并转发 WebSocket，前端 Dashboard 实时展示作业状态。
...
├── backend/        # 云端 Backend（工单 + WebSocket 转发）
...
```
agent（机器人端）  ──WebSocket──▶  backend（云端）  ──WebSocket──▶  front（浏览器）
     LangGraph 执行                  工单池 / 转发                   Dashboard
     硬件 / ROS（待接）               持久化（待接）
```
...
| **backend** | 云服务器 | 工单队列（权威源）、Agent/Dashboard WebSocket Hub、消息转发 |
| **shared** | 两端依赖 | `LiveEvent`、`WorkOrder`、消息 action 常量 |
...
./run_server.sh          # http://0.0.0.0:8000
...
pip install -r requirements.txt
python -m app.main run-cloud   # 默认 ws://127.0.0.1:8000/ws/agent
```
...
│   ├── reporter.py     # CloudReporter — 向云端上报 WebSocket 客户端
...
1. 连接 `/ws/agent`，发 `agent.hello`（`robotId`、`version`）
...
| 参数 | 默认 |
|------|------|
| `--cloud-url` | `ws://127.0.0.1:8000/ws/agent` |
| `--robot-id` | `TrayBot-01` |
...
FastAPI + WebSocket，不含 LangGraph。

### REST 端点

| 路径 | 方法 | 说明 |
|------|------|------|
| `/health` | GET | 健康检查（`agent_connected`、`dashboard_clients`） |
...
| 路径 | 方向 | 说明 |
|------|------|------|
| `/ws/agent` | Agent → Cloud | 端侧上报事件 |
| `/ws/dashboard` | Cloud → Front | 前端订阅（支持 `ping` → `pong`） |
